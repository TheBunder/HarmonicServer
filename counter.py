...
def record_to_file(path, key_to_stop):
    ...
    wf.close()
def main():
    sound_path = "sound_recording_copy.wav"
    recording_path = "recording_copy.wav"

    target_sound_file = "demo_sound_v2.wav"
    recording_file = "demo_record_v2.wav"
    similarity_threshold = 0.5

    # recourd the sound
    record_sound(target_sound_file)

    print("the sound has been record")

    # Record audio until the 'Q' key is pressed
    key_to_stop_recording = 'q'
    record_to_file(recording_file, key_to_stop_recording)

    print(
        "number of acourences: "
        + str(count_similar_sounds(target_sound_file, recording_file, 0.70))
    )
